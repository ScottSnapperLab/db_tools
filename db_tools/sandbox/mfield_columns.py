#!/usr/bin/env python
"""Columns created by Michael Field that need some adjustment."""
...
dob = Column(
    ...
    dtype=(str, type(None)),
    unique=False,
    validators=[redcap.validate.date_format],
...
wbc = Column(
    ...
    dtype=(str, type(None)),
    unique=False,
    validators=[redcap.validate.valid_float],
    recoders=[common.recode.nan_to_none]
)
...
def get_column_objects(rename=False):
    g = globals()
    if rename:
        return {val.name: val for key, val in g.items() if isinstance(val, Column)}
    else:
        return {key: val for key, val in g.items() if isinstance(val, Column)}


def get_column_name_translation_table():
    cols = get_column_objects()
    names = [[new_name, cols[new_name].name] for new_name in cols]
    return pd.DataFrame(names, columns=["mike_new", "mike_old"])


if __name__ == "__main__":

    table = get_column_name_translation_table()
    table.to_csv("mfield_columns.csv", index=False)